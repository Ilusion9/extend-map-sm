...
void AttemptVoteExtend(int client)
{
	...
	if (players < g_Cvar_MinPlayers.IntValue)
	{
		CReplyToCommand(client, "[SM] %t", "Min Players To Extend");
		return;
	}
	
	if (g_Extends >= g_Cvar_MaxExtends.IntValue)
	{
		CReplyToCommand(client, "[SM] %t", "Max Extends Reached");
		return;
	}
	...
	if (g_HasVoted[client])
	{
		CReplyToCommand(client, "[SM] %t", "Already Voted for Extend", g_Votes, requiredVotes);
		return;
	}
	...
	CPrintToChatAll("[SM] %t", "Vote for Extend Accepted", name, g_Votes, requiredVotes);
...
{
	CPrintToChatAll("[SM] %t", "Current Map Extended", g_Cvar_ExtendTime.IntValue);
